 ...
 */
	function sort($title, $key = null, $options = array()) {
		...
		if ($model == null) {
			$model = $this->__defaultModel.'.';
		}
// rajesh_04ag02 // sort header class
		$pos = strpos($this->options['url']['sort'], '.');
		if ($pos === false) {
			$sortModel = $this->__defaultModel.'.';
		} else {
			$sortModel = substr($this->options['url']['sort'], 0, $pos+1);
		}
		if ($model.$this->sortKey($options['model']) == $sortModel.$key && $this->sortDir($options['model']) == 'desc') {
...
// <--
		if ($model.$this->sortKey($options['model']) == $sortModel.$key && $this->sortDir($options['model']) == 'asc') {
			$dir = 'desc';
// rajesh_04ag02 // sort header class
            if (empty($options['class'])) {
                $options['class'] = 'asc';
