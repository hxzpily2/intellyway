 ...
 */
	function __createHeader() {
		...
		if (!empty($this->replyTo) && Configure::read('EmailTemplate.is_reply_to_email_enabled')) {
 ...
 */
	function __strip($value, $message = false) {
		$search = '%0a|%0d|Content-(?:Type|Transfer-Encoding)\:|charset\=|mime-version\:|multipart/mixed|(b?cc)\:.*';
		if ($message !== true) {
			$search .= '|\r|\n';
		}
		$search = '#(?:' . $search . ')#i';
