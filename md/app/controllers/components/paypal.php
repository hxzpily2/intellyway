...
class PaypalComponent extends Object
{
    ...
    public function logPaypalTransactions() 
    {
        ...
        $paypalTransactionModel = ClassRegistry::init($this->paypal_transaction_model);
        $this->data['PaypalTransactionLog']['paypal_response'] = $this->paypal_response;
        $this->data['PaypalTransactionLog']['error_no'] = $this->errno;
        $this->data['PaypalTransactionLog']['error_message'] = $errorMessaegString;
        if (!($this->errno == 0)) {
            $this->data['PaypalTransactionLog']['paypal_post_vars'] = $this->paypal_post_vars_in_str;
        }
        $this->data['PaypalTransactionLog']['ip'] = $this->RequestHandler->getClientIP();
        foreach($this->paypal_post_arr as $key => $value) {
            if (in_array($key, array(
                ...
                $this->data['PaypalTransactionLog']['is_authorization'] = 1;
            }
            $this->data['PaypalTransactionLog'][$key] = $value;
        }
        $paypalTransactionModel->save($this->data);
