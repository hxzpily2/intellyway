...
class DealUsersController extends AppController
{
    ...
    function index($deal_id = null)
    {
        ...
        if (empty($this->params['named']['type']) && empty($this->params['named']['deal_id']) && empty($this->data)) {
            // Available tab count //
            ...
            if ($this->Auth->user('user_type_id') != ConstUserTypes::Admin) {
                if ($this->Auth->user('user_type_id') == ConstUserTypes::Company && (!empty($this->params['named']['deal_id']) || !empty($deal_id))) {
			...
			//pr(array_merge($deal_conditions, $available_count_conditions));
            ...
            if (!empty($this->data['DealUser']['coupon_code'])) {
                $get_deal_user_id = $this->DealUser->DealUserCoupon->find('first', array(
                    'conditions' => array(
                        'OR' => array(
                            'DealUserCoupon.coupon_code like' => $this->data['DealUser']['coupon_code'] . '%',
                            'DealUserCoupon.unique_coupon_code like' => $this->data['DealUser']['coupon_code'] . '%',
                        )
                    ) ,
                    'recursive' => -1
                ));
            ...
            if (!empty($this->data['DealUser']['type'])) {
                $this->params['named']['type'] = $this->data['DealUser']['type'];
            }
            if ($this->Auth->user('user_type_id') == ConstUserTypes::Company && !empty($deal_id)) {
                ...
                if (!empty($deal) && ($deal['Deal']['deal_status_id'] == ConstDealStatus::Tipped || $deal['Deal']['deal_status_id'] == ConstDealStatus::Closed || $deal['Deal']['deal_status_id'] == ConstDealStatus::PaidToCompany)) {
                    $show_coupon_code = 1;
                    if (!empty($this->data['DealUser']['deal_user_view'])) {
                        $this->params['named']['deal_user_view'] = $this->data['DealUser']['deal_user_view'];
                    }
                }
            ...
            if (!empty($this->params['named']['type']) && $this->params['named']['type'] == 'available') {
                if ($this->Auth->user('user_type_id') != ConstUserTypes::Admin) {
                    if ($this->Auth->user('user_type_id') == ConstUserTypes::Company && !empty($this->params['named']['deal_id'])) {
                        $conditions['OR'] = array(
                            'Deal.user_id' => $this->Auth->user('id') ,
                            'Deal.company_id' => $company['Company']['id']
                        );
                ...
                $conditions['DealUser.is_canceled'] = 0;
            } elseif (!empty($this->params['named']['type']) && $this->params['named']['type'] == 'refund') {
                $conditions['DealUser.is_repaid'] = 1;
                $conditions['DealUser.is_canceled'] = 0;
            } elseif (!empty($this->params['named']['type']) && $this->params['named']['type'] == 'gifted_deals') {
                if ($this->Auth->user('user_type_id') != ConstUserTypes::Admin) {
                    if ($this->Auth->user('user_type_id') == ConstUserTypes::Company && !empty($this->params['named']['deal_id'])) {
            ...
            } elseif (!empty($this->params['named']['type']) && $this->params['named']['type'] == 'recieved_gift_deals') {
                $conditions = array();
                ...
                $conditions['DealUser.is_canceled'] = 0;
            } elseif (!empty($this->params['named']['type']) && $this->params['named']['type'] == 'canceled') {
            ...
            if (!empty($this->params['named']['type'])) {
                $this->data['DealUser']['type'] = $this->params['named']['type'];
            }
            $this->paginate = array(
    ...
    function view($id = null)
    {
        ...
        if (!empty($this->params['named']['coupon_id'])) {
            $coupon_conditions['DealUserCoupon.id'] = $this->params['named']['coupon_id'];
        }
        if (!empty($this->params['named']['filter_id'])) {
            ...
            if (!empty($this->params['named']['filter_id']) && $this->params['named']['filter_id'] == 'available') {
                ...
                $conditions['DealUser.deal_user_coupon_count !='] = 0;
            } elseif (!empty($this->params['named']['filter_id']) && $this->params['named']['filter_id'] == 'expired') {
                $conditions['Deal.coupon_expiry_date <='] = _formatDate('Y-m-d H:i:s', date('Y-m-d H:i:s') , true);
                $conditions['DealUser.is_repaid'] = 0;
        ...
        if (!empty($this->params['named']['type']) && $this->params['named']['type'] == 'print') {
            $this->layout = 'print';
        }
    ...
    function admin_index()
    {
        ...
        if (isset($this->params['named']['deal_name'])) {
            ...
            $conditions['Deal.name'] = $this->params['named']['deal_name'];
            $this->pageTitle.= sprintf(__l(' - Search - %s') , $this->params['named']['deal_name']);
        }
        if (!empty($this->params['named']['coupon_code'])) {
            ...
            $conditions['DealUser.id'] = $get_deal_user_id['DealUserCoupon']['deal_user_id'];
            $coupon_find_id[] = $get_deal_user_id['DealUserCoupon']['id'];
            $this->pageTitle.= sprintf(__l(' - Search - %s') , $this->params['named']['coupon_code']);
        }
        ...
        if (!empty($this->params['named']['filter_id']) && $this->params['named']['filter_id'] == 'available') {
            ...
            $conditions['Deal.deal_status_id'] = ConstDealStatus::Open;
        } elseif (!empty($this->params['named']['filter_id']) && $this->params['named']['filter_id'] == 'canceled') {
            $conditions['DealUser.is_canceled'] = 1;
        } elseif (!empty($this->params['named']['filter_id']) && $this->params['named']['filter_id'] == 'refunded') {
            $conditions['Deal.deal_status_id'] = ConstDealStatus::Refunded;
            $conditions['DealUser.is_repaid'] = 1;
        } elseif (!empty($this->params['named']['filter_id']) && $this->params['named']['filter_id'] == 'all') {
            unset($conditions['DealUser.is_canceled']);
            unset($conditions['DealUser.is_repaid']);
        } else {
            unset($conditions['DealUser.is_repaid']);
        }
        if (!empty($this->params['named']['deal_name'])) {
            $this->data['DealUser']['deal_name'] = $this->params['named']['deal_name'];
            $this->pageTitle.= sprintf(__l(' - Search - %s') , $this->params['named']['deal_name']);
            $conditions['Deal.name'] = $this->data['DealUser']['deal_name'];
        }
        if (!empty($this->params['named']['filter_id'])) {
