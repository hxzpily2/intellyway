		...
		<?php foreach($user_deals as $user_deal){ ?>
			...
			<li class = "clearfix <?php echo $class;?>">
				<div class="company-list-image">
					<?php echo $html->showImage('Deal', $user_deal['Deal']['Attachment'][0], array('dimension' => 'medium_thumb', 'alt' => sprintf(__l('[Image: %s]'), $html->cText($user_deal['Deal']['name'], false)), 'title' => $html->cText($user_deal['Deal']['name'], false)));?>
				</div>
				<div class="company-list-content">
					<h3><?php echo $html->link($user_deal['Deal']['name'], array('controller' => 'deals', 'action' => 'view', $user_deal['Deal']['slug']),array('title' => sprintf(__l('%s'),$user_deal['Deal']['name'])));?></h3>
