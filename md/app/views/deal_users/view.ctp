...
<tr>
      <td width="70%" style="padding:10px 0px;">
		<?php echo $html->image(Router::url(array(
					'controller' => 'img',
					'action' => 'blue-theme',
					'logo-black.png',
					'admin' => false
      ...
      <td width="30%" style="padding:10px 0px;">
			...
			<?php echo '#'.$html->cText($deal_user_coupon['coupon_code']);?>
			</strong>
      </td>
    </tr>
    <tr>	
		...
		<p>
			<strong style="color:#000; font-size:20px;display:block;margin:15px 0px 10px 0px; "><?php echo $html->cText($dealUser['Deal']['name']);?></strong>
		</p>
      <div  style=" margin:0px,padding:10px;">
		...
		<tr>
			<td style="width: 49%;line-height:22px;vertical-align:top;color:#000;">
				...
				<p style="font-family:arial;font-size:13px;margin:0px;padding:0px;">	<?php echo !empty($dealUser['DealUser']['gift_to']) ? $html->cText($dealUser['DealUser']['gift_to']) : $html->cText($dealUser['User']['username']);?></p>
				<strong style="color:#000; font-size4:16px;display:block;margin:5px 0px 0px 0px;"><?php echo __l('Valid From:');?></strong>
					<p style="font-family:arial;font-size:13px;margin:0px;padding:0px;"> <?php echo $html->cDateTime($dealUser['Deal']['coupon_start_date']);?></p>
				<?php if(empty($dealUser['Deal']['is_anytime_deal'])):?>
					<strong style="color:#000; font-size4:16px;display:block;margin:5px 0px 0px 0px;"><?php echo __l('Expires:');?></strong>
						<p style="font-family:arial;font-size:13px;margin:0px;padding:0px;"> <?php echo $html->cDateTime($dealUser['Deal']['coupon_expiry_date']);?></p>
				<?php endif;?>	
				<strong style="color:#000; font-size:16px;display:block;margin:6px 0px 0px 0px; padding:0px 0px 0px 0px"><?php echo __l('Fine Print:');?></strong>					
				  	<p style="font-family:arial;font-size:13px;margin:0px;padding:0px;"> <?php echo $html->cText($dealUser['Deal']['coupon_condition']);?></p>
...
<td style="width: 49%;line-height:22px; color:#000;" valign="top">
				<dl style="margin:0px;padding:0px;">
				  ...
				  <dd style="width:157px;float:left;margin:0px;padding:0px 0px 0px 10px; font-family:arial;font-size:13px;">
					<?php echo $html->cText($dealUser['Deal']['Company']['name']);?><br />
					<?php if(!empty($dealUser['Deal']['Company']['address1'])) { ?>
						<?php echo $html->cText($dealUser['Deal']['Company']['address1']);?><br />
					<?php } ?>
					<?php if(!empty($dealUser['Deal']['Company']['address2'])) { ?>
						<?php echo $html->cText($dealUser['Deal']['Company']['address2']);?><br />
					<?php } ?>
					<?php if(!empty($dealUser['Deal']['Company']['City']['name'])) { ?>
						<?php echo $html->cText($dealUser['Deal']['Company']['City']['name']);?><br />
					<?php } ?>
					<?php if(!empty($dealUser['Deal']['Company']['State']['name'])) { ?>
						<?php echo $html->cText($dealUser['Deal']['Company']['State']['name']);?><br />
					<?php } ?>
					<?php if(!empty($dealUser['Deal']['Company']['Country']['name'])) { ?>
						<?php echo $html->cText($dealUser['Deal']['Company']['Country']['name']);?><br />
					<?php } ?>
					<?php if(!empty($dealUser['Deal']['Company']['zip'])) { ?>
						<?php echo $html->cText($dealUser['Deal']['Company']['zip']);?><br/>
					<?php } ?>
				  </dd>
				  ...
				  <dd style="width:157px;float:left;margin:0px;padding:0px 0px 0px 10px;font-family:arial;font-size:13px;"><?php echo $html->cDateTime($dealUser['DealUser']['created']);?></dd>
				</dl>
	...
	<?php if(!empty($dealUser['DealUser']['is_gift'])):?>
		...
		<td width="80%" style="margin:0px;padding:0px;">
		   <p><?php echo $html->image(Router::url(array(
					'controller' => 'img',
					'action' => 'gift.png',
					'admin' => false
				),true), array('alt'=> __l('[Image: Logo]'), 'title' => Configure::read('site.name'))); ?>
    ...
    <table style="margin:10px 0px 0px 0px;padding:10px;">
        ...
        <td width="60%" style="vertical-align:top;">
          	<strong style="color:#000;padding:10px 0px 0px 0px;font-size: 16px; "><?php echo __l('How to use this:');?></strong><br/>
				...
				<?php echo __l('customers tip on the full
				...
				<td width="40%" style="vertical-align:top;">
					<strong style="color:#000; font-size:16px;padding:10px 0px 0px 0px;"><?php echo __l('Map:');?></strong>
						...
						<?php 
							if(!empty($this->params['named']['type']) == 'print'):
								echo $html->image($html->formGooglemap($dealUser['Deal']['Company'],'320x250'));
							else:
								if(Configure::read('GoogleMap.embedd_map') == 'Static'):
									echo $html->image($html->formGooglemap($dealUser['Deal']['Company'],'320x250'));
								else:
									echo $html->formGooglemap($dealUser['Deal']['Company'],'320x250');
								endif;	
								?>
...
?>
<?php if(!empty($this->params['named']['type']) && $this->params['named']['type'] == 'print'): ?>
	<script>
         window.print();
    </script>
<?php endif; ?>
