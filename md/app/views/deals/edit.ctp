<?php /* SVN: $Id: edit.ctp 44816 2011-02-19 12:02:30Z aravindan_111act10 $ */ ?>
...
<?php echo $form->create('Deal', array('class' => 'normal js-upload-form {is_required:"false"}', 'enctype' => 'multipart/form-data'));?>
	...
	<div class="js-validation-part">
       <fieldset class="form-block round-5">
        ...
        <?php echo $html->link($html->showImage('Deal', $this->data['Attachment'][0], array('dimension' => 'normal_thumb', 'alt' => sprintf(__l('[Image: %s]'), $html->cText($this->data['Deal']['name'], false)), 'title' => $html->cText($this->data['Deal']['name'], false))), array('controller' => 'deals', 'action' => 'view',  $this->data['Deal']['slug'], 'admin' => false), null, null, false); ?>
		<?php
            echo $form->input('id');
	...
	<div class="clearfix date-time-block">
		...
		?>		
            ...
            <?php echo $form->input('original_price',array('class' => 'js-price', $currecncy_place => Configure::read('site.currency'))); ?>
       ...
       <legend class="round-5"><?php echo __l('Commission'); ?></legend> 
        <div class="page-info">
        	<?php
				echo __l('Total Commission Amount = Bonus Amount + ((Discounted Price * Number of Minimum Buyers) * Commission Percentage/100))');
			 ?>
        </div>
		<div class="clearfix">
	   <div class="amount-block commision-form-block">
        ...
        <?php
		if($auth->user('user_type_id') == ConstUserTypes::Admin):		
			echo $form->input('is_coupon_mail_sent',array('label' => __l('Coupon Mail Sent to Users')));
	   ...
	   <fieldset class="form-block round-5">
		...
		<?php
			echo $form->uploader('Attachment.filename', array('type'=>'file', 'uController' => 'deals', 'uRedirectURL' => array('controller' => 'deals', 'action' => 'index', 'admin' => false), 'uId' => 'dealID', 'uFiletype' => Configure::read('photo.file.allowedExt')));
		?>
		</fieldset>
	   <fieldset class="form-block round-5">
    ...
    <div class="submit-block clearfix">
            ...
            <?php else:
                ...
                <div class="cancel-block">
                    <?php echo $html->link(__l('Cancel'), array('controller' => 'deals', 'action' => 'index', 'company' => $company_slug), array('class' => 'cancel-button')); ?>
                </div>
            <?php endif; ?>
   </div>
   <?php echo $form->end();	?>
