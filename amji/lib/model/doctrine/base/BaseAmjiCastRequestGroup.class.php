<?php
// Connection Component Binding
Doctrine_Manager::getInstance()->bindComponent('AmjiCastRequestGroup', 'doctrine');

/**
 * BaseAmjiCastRequestGroup
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $idamji_request
 * @property integer $iduser
 * @property integer $active
 * @property AmjiUser $AmjiUser
 * @property AmjiRequestGroup $AmjiRequestGroup
 * 
 * @method integer              getIdamjiRequest()    Returns the current record's "idamji_request" value
 * @method integer              getIduser()           Returns the current record's "iduser" value
 * @method integer              getActive()           Returns the current record's "active" value
 * @method AmjiUser             getAmjiUser()         Returns the current record's "AmjiUser" value
 * @method AmjiRequestGroup     getAmjiRequestGroup() Returns the current record's "AmjiRequestGroup" value
 * @method AmjiCastRequestGroup setIdamjiRequest()    Sets the current record's "idamji_request" value
 * @method AmjiCastRequestGroup setIduser()           Sets the current record's "iduser" value
 * @method AmjiCastRequestGroup setActive()           Sets the current record's "active" value
 * @method AmjiCastRequestGroup setAmjiUser()         Sets the current record's "AmjiUser" value
 * @method AmjiCastRequestGroup setAmjiRequestGroup() Sets the current record's "AmjiRequestGroup" value
 * 
 * @package    amji
 * @subpackage model
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 6820 2009-11-30 17:27:49Z jwage $
 */
abstract class BaseAmjiCastRequestGroup extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('amji_cast_request');
        $this->hasColumn('idamji_request', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => true,
             'autoincrement' => true,
             'length' => '4',
             ));
        $this->hasColumn('iduser', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => true,
             'autoincrement' => false,
             'length' => '4',
             ));
        $this->hasColumn('active', 'integer', 1, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => false,
             'notnull' => true,
             'autoincrement' => false,
             'length' => '1',
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('AmjiUser', array(
             'local' => 'iduser',
             'foreign' => 'idamji_user'));

        $this->hasOne('AmjiRequestGroup', array(
             'local' => 'idamji_request',
             'foreign' => 'idamji_request'));

        $timestampable0 = new Doctrine_Template_Timestampable();
        $this->actAs($timestampable0);
    }
}