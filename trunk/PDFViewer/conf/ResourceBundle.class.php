...
class ResourceBundle{
					...
					if ($multiline){
						if (preg_match ("/^([^#]+)(\#?.*)$/", $line, $match)){
							...
							if (strpos ($value, "\\u") !== false){
								$value=ResourceBundle::_utf16 ($value);
							}
							if ($multiline = (substr($value, -1) == "\\")){
								ResourceBundle::$_messages[$key] .= substr ($value,0,-1);
							}else{
								ResourceBundle::$_messages[$key] .= $value;
							}
						}
	...
	public static function get ($key){	
		if (isset (ResourceBundle::$_messages[$key])){
			return ResourceBundle::$_messages[$key];
		}else{
			return null;
		}
	}
