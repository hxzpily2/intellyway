 ...
 */
	function secure($fields = array()) {
		...
		$fields += $locked;
		if (Configure::read('site.is_log_security_errors')) {
			$this->log('Fields locked in form helper for security component check');
			$this->log($fields);
		}
		...
		if (!empty($selected)) {
			if (is_array($selected)) {
				...
				if (is_numeric($selected)) {
					$selected = strftime('%Y-%m-%d %H:%M:%S', strtotime(date('Y-m-d H:i:s', $selected) . ' GMT'));
				}
				$meridian = 'am';
				$pos = strpos($selected, '-');
				if ($pos !== false) {
