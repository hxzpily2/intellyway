(function($){
	...
	$.jmap.init = function(el, options, callback) {
		...
		/* add zoom listener*/
			GEvent.addListener(el.jmap,"mouseover", function(overlay,point) {
			   if(document.getElementById('zoomlevel'))
			   document.getElementById('zoomlevel').value= el.jmap.getZoom();
		  });
	      GEvent.addListener(el.jmap,"mouseout", function(overlay,point) {
			   if(document.getElementById('zoomlevel'))
			   document.getElementById('zoomlevel').value= el.jmap.getZoom();
		  });
         /* add zoom listener*/ 
	...
	$.jmap.addMarker = function(options, callback) {
		...
		if($.jmap.GMarkerManager) {
			...
			//Codes added to get the current latitude and longitude value while dragging on the map
			 marker.id = 'marker';
			 $.jmap.GMap2.addOverlay(marker);GEvent.addListener(marker, "dragend", function()
			 { 
			 document.getElementById('latitude').value=marker.getPoint().lat();
			 document.getElementById('longitude').value=marker.getPoint().lng() });
		}
