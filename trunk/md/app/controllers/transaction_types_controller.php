    ...
    function admin_edit($id = null)
    {
        ...
        if (!empty($this->data)) {
            if ($this->TransactionType->save($this->data)) {
                $this->Session->setFlash(sprintf(__l('"%s" Transaction Type has been updated') , $this->data['TransactionType']['name']) , 'default', null, 'success');
                $this->redirect(array(
                    'action' => 'index'
                ));
            } else {
                $this->Session->setFlash(sprintf(__l('"%s" Transaction Type could not be updated. Please, try again.') , $this->data['TransactionType']['name']) , 'default', null, 'error');
            }
        } else {
